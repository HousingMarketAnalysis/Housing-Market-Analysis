...
def PROPOSED(Population, objective_function, lb, ub, Max_iter):
    # Improved Quokka Swarm Optimization (IQSO)
    # Proposed Position update is done at line 47
    ...
    for iteration in range(Max_iter):
        ...
        for i in range(Pop_Size):
            ...
            # r = np.random.rand()
            r = Fitness_Values[i] / (2 * np.max(Fitness_Values) + np.mean(Fitness_Values))
